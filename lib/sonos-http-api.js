...
function HttpAPI(discovery, settings) {
  ...
  async function invokeWebhook(type, data) {
    ...
    let albumArtBase64 = '';
    if (data.state) {
      albumArtBase64 = await getLocalAlbumArt(
        data.state.currentTrack.absoluteAlbumArtUri
      );
    }

    const jsonBody = JSON.stringify({
      [typeName]: type,
      [dataName]: Object.assign({ albumArtBase64 }, data),
    });
...
function getLocalAlbumArt(absoluteAlbumArtUri) {
  return new Promise((resolve, reject) => {
    req.get(absoluteAlbumArtUri, (error, response, body) => {
